    ...
    <div class="topbar">
        ...
        <div class="ms-auto d-flex align-items-center gap-3">
            ...
            <div class="position-relative" id="notifBellWrapper">
                <button class="btn btn-sm btn-outline-secondary position-relative" id="notifBellBtn"
                ...
                <div class="dropdown-menu dropdown-menu-end shadow" style="width:340px;max-height:420px;overflow-y:auto" id="notifDropdown">
                    ...
                    <div id="notifList">
                        <div class="text-center text-muted small py-4"><i class="fas fa-bell-slash me-1"></i>Tidak ada notifikasi</div>
                    </div>
                </div>
            </div>
        ...
        <div class="card shadow-sm mb-4">
            ...
            <div class="card-body">
                <div class="row g-3">
                    ...
                    <div class="col-6 col-md-2">
                        <a href="analytics.php" class="qaction bg-danger bg-opacity-10 text-danger border-danger border-opacity-25">
...
// ─── Notifications ────────────────────────────────────────────────────────────
const typeIcon = { approval_pending:'tasks', approval_done:'check-circle', loan_applied:'hand-holding-usd', payment_due:'exclamation-triangle', member_registered:'user-plus', system:'cog', report_ready:'file-alt' };
const typeColor = { approval_pending:'warning', approval_done:'success', loan_applied:'primary', payment_due:'danger', member_registered:'info', system:'secondary', report_ready:'success' };

function pollNotifications() {
    apiPost('/notifications.php', { action: 'poll' })
        .done(function (d) {
            if (!d.success) return;
            const count = d.data.unread_count || 0;
            ...
            if (!notifs.length) {
                $('#notifList').html('<div class="text-center text-muted small py-4"><i class="fas fa-check-circle text-success me-1"></i>Semua sudah dibaca</div>');
                return;
            }

            $('#notifList').html(notifs.map(n => `
                <div class="dropdown-item d-flex gap-2 py-2 ${n.is_read ? 'opacity-60' : 'bg-light bg-opacity-50'}" style="white-space:normal;border-bottom:1px solid #f3f4f6">
                    <div class="mt-1 flex-shrink-0">
                        <span class="badge bg-${typeColor[n.type] || 'secondary'} rounded-circle" style="width:28px;height:28px;display:flex;align-items:center;justify-content:center">
                            <i class="fas fa-${typeIcon[n.type] || 'bell'}" style="font-size:.65rem"></i>
                        </span>
                    </div>
                    <div class="flex-grow-1 overflow-hidden">
                        <div class="fw-semibold" style="font-size:.8rem">${n.title}</div>
                        <div class="text-muted" style="font-size:.75rem">${n.message}</div>
                        <div class="text-muted mt-1" style="font-size:.7rem">${new Date(n.created_at).toLocaleString('id-ID',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'})}</div>
                    </div>
                    ${n.link ? `<a href="${n.link}" class="btn btn-xs btn-sm btn-outline-primary py-0 px-1 flex-shrink-0" style="font-size:.7rem">Buka</a>` : ''}
                </div>`).join(''));
        }).fail(() => {});
}

function markAllRead() {
    apiPost('/notifications.php', { action: 'mark_all' })
        .done(function () {
            $('#notifBadge').hide();
            $('#notifList').html('<div class="text-center text-muted small py-4"><i class="fas fa-check-circle text-success me-1"></i>Semua sudah dibaca</div>');
        });
...
$(document).ready(function () {
    refreshAll();
    pollNotifications();
    setInterval(function () { loadApprovalStats(); loadPendingApprovals(); pollNotifications(); }, 30000);
});
</script>
</body>
</html>
