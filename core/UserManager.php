        ...
        switch ($role) {
            case 'bos':
                return [
                    ...
                    'total_deposits' => $db->fetchOne("SELECT SUM(amount) as total FROM savings")['total'],
                    'total_loans' => $db->fetchOne("SELECT SUM(loan_amount) as total FROM loans")['total'],
                    'total_users' => $db->fetchOne("SELECT COUNT(*) as count FROM users")['count']
                ];
                
            case 'admin':
                return [
                    'active_members' => $db->fetchOne("SELECT COUNT(*) as count FROM members")['count'],
                    'pending_loans' => $db->fetchOne("SELECT COUNT(*) as count FROM loans")['count'],
            ...
            case 'teller':
                return [
                    'today_transactions' => $db->fetchOne("SELECT COUNT(*) as count FROM transactions WHERE DATE(created_at) = CURDATE() AND created_by = ?", [$this->user['id']])['count'],
                    'total_amount' => $db->fetchOne("SELECT SUM(amount) as total FROM transactions WHERE DATE(created_at) = CURDATE() AND created_by = ?", [$this->user['id']])['total'],
                    'pending_verifications' => $db->fetchOne("SELECT COUNT(*) as count FROM transactions WHERE status = 'pending' AND created_by = ?", [$this->user['id']])['count'],
                    'new_members_today' => $db->fetchOne("SELECT COUNT(*) as count FROM members WHERE DATE(created_at) = CURDATE()")['count']
                ];
                
            case 'collector':
                return [
                    'today_collections' => $db->fetchOne("SELECT COUNT(*) as count FROM transactions WHERE DATE(created_at) = CURDATE() AND created_by = ?", [$this->user['id']])['count'],
                    'collection_amount' => $db->fetchOne("SELECT SUM(amount) as total FROM transactions WHERE DATE(created_at) = CURDATE() AND created_by = ?", [$this->user['id']])['total'],
                    'pending_collections' => $db->fetchOne("SELECT COUNT(*) as count FROM loans WHERE status = 'active' AND due_date <= CURDATE()", [])['count'],
                    'active_routes' => $db->fetchOne("SELECT COUNT(*) as count FROM members WHERE status = 'active'", [])['count']
                ];
                
            default:
                // Only member-specific roles should access member data
                if (isset($this->user['member_id'])) {
                    return [
                    ...
                    // Fallback for roles without member_id
                    return [
                        'account_balance' => 0,
                        'active_loans' => 0,
                        'pending_payments' => 0,
