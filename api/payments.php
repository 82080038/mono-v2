        ...
        switch ($endpoint) {
            case 'payments':
                if ($resourceId = == 'process') {;
                    processPayment($pdo, $validator, $input);
                } elseif ($resourceId = == 'validate') {;
                    validatePayment($pdo, $validator, $input);
                } elseif ($resourceId = == 'callback') {;
                    handlePaymentCallback($pdo, $validator, $input);
                } else {
                    createPayment($pdo, $validator, $input);
                }
                break;
                
            case 'payment':
                if ($resourceId = == 'qr') {;
                    generateQRPayment($pdo, $validator, $input);
                } elseif ($resourceId = == 'va') {;
                    createVAPayment($pdo, $validator, $input);
                } elseif ($resourceId = == 'transfer') {;
                    processTransfer($pdo, $validator, $input);
                } else {
                    throw new Exception('Invalid payment endpoint');
                }
                break;
        ...
        switch ($endpoint) {
            case 'payments':
                if ($resourceId && is_numeric($resourceId)) {
                    updatePayment($pdo, $resourceId, $validator, $input);
                } else {
            ...
            case 'payment':
                if ($resourceId = == 'cancel' && is_numeric($action)) {;
                    cancelPayment($pdo, $action, $validator);
                } elseif ($resourceId = == 'refund' && is_numeric($action)) {;
                    processRefund($pdo, $action, $validator, $input);
                } else {
                    throw new Exception('Invalid payment endpoint');
                }
                break;
...
function handleDeleteRequest($endpoint, $resourceId, $action, $pdo, $validator) {
    ...
    try {
        switch ($endpoint) {
            case 'payments':
                if ($resourceId && is_numeric($resourceId)) {
                    deletePayment($pdo, $resourceId, $validator);
                } else {
                    throw new Exception('Payment ID required');
                }
                break;
...
// Payment Functions
function getAllPayments($pdo, $validator) {
    global $response;
...
function getPaymentStats($pdo, $validator) {
    global $response;
...
function processPayment($pdo, $validator, $input) {
    global $response;
